  ...
  const pageButtonClass = (isActive: boolean) =>
    [
      ...
      isActive
        ...
        : "bg-[#FDE4C8] text-[#2D2B2B] hover:bg-[#3d3a3a] hover:text-[#D9D9D9]!",
    ].join(" ");

  const navButtonClass =
    "godly-button rounded-sm inline-flex w-fit min-w-0 items-center gap-2 px-4 py-2.5 disabled:opacity-100! disabled:cursor-not-allowed!";

  const pageButtonMobileClass = (isActive: boolean) =>
    [
      ...
      isActive
        ? "bg-[#2D2B2B] text-[#D9D9D9]!"
        : "bg-[#FDE4C8] text-[#2D2B2B] active:bg-[#e8d4b8]",
    ].join(" ");

  return (
    <>
      <div className="flex flex-col bg-paper-16-3 px-[35px] py-[100px] md:p-16 md:py-[70px]">
        <div className="mx-auto flex w-full max-w-[1311px] flex-col items-center gap-2">
          <h1
            className="trim font-marlton text-center text-[50px] text-[#FDE4C8] md:text-[96px]"
            style={{ textShadow: "4px 0px 0px #AF8F6E" }}
          >
            {cityName ? `${cityName} resources` : "Holiday resources"}
          </h1>
      ...
      <div className="flex flex-col gap-10 bg-[#fef7ea] md:pb-16">
        {posts.length === 0 && (
          <div className="mx-auto flex w-full max-w-[1311px] flex-col items-center gap-3 px-[30px] py-[80px] md:px-12">
            ...
            <p className="text-center font-satoshi text-[20px] font-light text-[#2D2B2B]/75">
          ...
          <>
            <div className="hidden pt-[40px] md:block md:px-12">
              <div className="mx-auto flex max-w-[1311px] flex-col items-start gap-8 border-black pb-6 md:flex-row">
                <div className="w-full md:w-1/2">
                  <div className="mx-auto aspect-4/3 max-w-[650px] overflow-hidden border-8 border-[rgba(106,100,100,0.12)] shadow-md">
                    <Link
                ...
                <div className="w-full md:w-1/2">
                  <p className="mb-1 font-satoshi text-lg font-light text-[#2D2B2B]/70">
                    {posts[0].publishedAt
                      ? format(new Date(posts[0].publishedAt), "MMMM d, yyyy")
                      : ""}
                  </p>
                  <h2 className="mb-4 font-marlton text-[36px] font-normal text-[#0E0E0E]">
                    {posts[0].title}
                  </h2>
                  <p className="mb-4 line-clamp-9 font-satoshi text-[20px] font-light leading-[31px] text-[#2D2B2B]">
                    {posts[0].excerpt ?? ""}
                  </p>
                  <div className="flex w-full justify-end">
                    <Link
                      href={blogPostHref(posts[0].slug)}
                      className="font-marlton text-2xl text-[#2D2B2B] underline decoration-[#AF8F6E] underline-offset-4 transition-colors hover:text-[#AF8F6E]"
                    >
                      READ MORE
                    </Link>
                  </div>
            ...
            <div className="flex flex-col gap-[24px] px-[30px] py-[52px] md:hidden">
              <div className="flex w-full flex-col gap-3">
                <h2 className="trim font-marlton text-[32px] font-normal text-[#0E0E0E]">
                  {posts[0].title}
                </h2>
                <p className="font-satoshi text-sm font-light leading-4 text-[#2D2B2B]/80">
                  {posts[0].publishedAt
                    ? format(new Date(posts[0].publishedAt), "MMMM d, yyyy")
                    : ""}
                </p>
                ...
                >
                  <div className="aspect-4/3 w-[334px] overflow-hidden border-[6px] border-[rgba(106,100,100,0.12)] shadow-md">
                    <Image
                      src={posts[0].image}
                      alt={posts[0].title}
                      width={334}
              ...
              <div>
                <p className="line-clamp-8 font-satoshi text-xl font-light leading-[26px] text-[#2D2B2B]">
                  {posts[0].excerpt ?? ""}
                </p>
              </div>
              <div>
                <div className="flex w-full justify-end">
                  <Link
                    href={blogPostHref(posts[0].slug)}
                    className="font-marlton text-xl leading-6 text-[#2D2B2B] underline decoration-[#AF8F6E] underline-offset-4 transition-colors hover:text-[#AF8F6E]"
                  >
                    READ MORE
                  </Link>
                </div>
        ...
        {posts[0] && !posts[0].image && (
          <div className="mx-auto max-w-[1311px] px-[30px] py-[40px] md:px-12">
            <article className="flex flex-col gap-3 border-b border-[rgba(106,100,100,0.12)] pb-6">
              <Link
                href={blogPostHref(posts[0].slug)}
                className="flex flex-col gap-3"
              >
                <p className="font-satoshi text-base font-light text-[#2D2B2B]/80">
                  {posts[0].publishedAt
                    ? format(new Date(posts[0].publishedAt), "MMMM d, yyyy")
                    : ""}
                </p>
                <h3 className="font-marlton text-2xl leading-snug font-normal text-[#0E0E0E] underline decoration-[#2D2B2B] transition-colors hover:text-[#AF8F6E] hover:decoration-[#AF8F6E] md:text-4xl">
                  {posts[0].title}
                </h3>
                <span className="font-satoshi text-sm font-medium text-[#AF8F6E] underline">
                  Read more →
                </span>
              </Link>
            </article>
                  ...
                  <div className="mx-auto flex w-max justify-center gap-2 py-1">
                    {paginationItems.map((item, i) =>
                      item === null ? (
                        <span
                          key={`m-ellipsis-${i}`}
                          className="font-marlton flex shrink-0 snap-center items-center px-1 text-[#2D2B2B]"
                          aria-hidden
                        >
                          …
                        </span>
              ...
              <div className="flex flex-wrap items-center justify-center gap-[7px]">
                ...
                {paginationItems.map((item, i) =>
                  item === null ? (
                    <span
                      key={`ellipsis-${i}`}
                      className="font-marlton trim px-1 text-[#2D2B2B]"
                      aria-hidden
                    >
                      …
                    </span>
        ...
        {showPagination && (
          <div className="mx-auto hidden w-full max-w-[1400px] space-y-0.25 px-[30px] md:block md:px-12">
            <div className="h-px w-full bg-[#2D2B2B]" />
            <div className="h-[3px] w-full bg-[#2D2B2B]" />
          </div>
        )}
      </div>
    </>
